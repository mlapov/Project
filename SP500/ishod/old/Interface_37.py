...
class TelegraBot(Thread):
    def __init__(self, interactive):
        ...
        if self.interactive == "interactive":
            ...
            # def start(m, res=False):
            #     self.bot.send_message(m.chat.id, 'Пока. Интерактив')
            #     print('Пока1. Интерактив')
            #     # self.bot.stop_polling()
            #     print('Пока2. Интерактив')
            #     self.bot.stop_bot()
    ...
    def run(self):
        print(f"from Telegram: {self.interactive}]")
        if self.interactive == "interactive":
            # self.bot.infinity_polling(long_polling_timeout=2)
            self.bot.polling(none_stop=True, interval=0, long_polling_timeout=2)  #
        print(f"from Telegram: 1111")

        while self.flag_run_telegram:
            print(f"from Telegram: {self.i}")
            self.i += 1
            time.sleep(0.5)
            ...
            # self.bot.stop_bot()
            # self.bot.stop_polling()
        print(f"закрыли {self.interactive}" )
        # try:
        #     tB2.bot.close()  #stop_bot()
        # except:
        #     print("пытаемся закрыть второго бота")
        #     tB2.join()  #bot.stop_bot()   #close()  # stop_bot()
        # tB1.bot.close()  #.stop_bot()
        # tB2.bot.close()  #stop_bot()
...
if __name__ == '__main__':
    ...
    wS = WinService()
    sE = SendEmail()
    tB1 = TelegraBot("non_interactive")
    tB2 = TelegraBot("interactive")
    rb = robot.RobotApp(mainwindow=wU, servicewindow=wS, email=sE, telegram=tB1, telegram_interactive=tB2)  # будем получать доступ к Окну
