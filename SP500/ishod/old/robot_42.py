                    ...
                    if flag_account_true:  # если аккаунт валидный
                        ...
                        if self.flag_orderStatus:
                            ...
                            if self.param_orderStatus["status"] == "Filled" and self.param_orderStatus[
                                ...
                                self.MessageInfo(f'Сделка на {self.param_orderStatus["filled"]} контракт(ов) завершена!')
                        ...
                        if self.flag_contractDetails:
                            ...
                            self.MessageInfo(f"Последний торговый день бумаги {self.contract_1['localSymbol']} - {tmp_str[6:8]}.{tmp_str[4:6]}.{tmp_str[0:4]}")
                        ...
                        if time_paper.day != day_old:
                            ...
                            now = time_paper.date()
                            then = datetime(int(self.lastTradeDate[0:4]), int(self.lastTradeDate[4:6]), int(self.lastTradeDate[6:8])).date()
                            delta_d = then - now
