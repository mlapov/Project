                ...
                while self.isConnected() or not self.msg_queue.empty():
                    ...
                    if flag_account_true:  # если аккаунт валидный
                        ...
                        if time_paper:  # контракт читали и время бумаги уже не None
                            ...
                            if not self.flag_Position:
                                if time_paper.hour == int(self.time_to_act['input_hour']) \
                                        ...
                                        and not flag_one_trade:  # юда пока не заходили
                                    ...
                                    number_of_lot = 1  # кол-во лотов, рассчитывается от риска - НАДО!!!

                                    # расчитываем цену для входа
                        ...
                        if self.flag_execDetails:  # сделка совершена на кол-во лотов  = execution.shares
                            self.flag_execDetails = False
                            # log_r.info(f'Trade: {self.param_execDetails["execution"].shares} contracts,'
                            #            f'Contract - {self.param_execDetails["contract"].localSymbol}')
                            # self.MessageInfo(f'Trade: {self.param_execDetails["execution"].shares} contracts,'
                            #                  f'Contract - {self.param_execDetails["contract"].localSymbol}')

                        if self.flag_orderStatus:
                            self.flag_orderStatus = False
                            # log_r.info(f'OrderStatus: Status {self.param_orderStatus["status"]}, '
                            ...
                            # self.MessageInfo(f'OrderStatus: Status {self.param_orderStatus["status"]}, '
                            #                  f'Filled:{self.param_orderStatus["filled"]}, '
                            #                  f'Remaining:{self.param_orderStatus["remaining"]}'
                            #                  f'LastFillPrice: {self.param_orderStatus["lastFillPrice"]}')
                        ...
                        if self.flag_openOrder:
                            self.flag_openOrder = False
                            # log_r.info(f'OpenOrder: direction {self.param_openOrder["order"].action}, '
                            #            f'status{self.param_openOrder["order"].status}')
                            # self.MessageInfo(f'OpenOrder: direction {self.param_openOrder["order"].action}, '
                            #                  f'status{self.param_openOrder["order"].status}')
                            # print("OpenOrder. ID:", orderId, contract.symbol, contract.secType, "@", contract.exchange,
                            #       ":",
                            #       order.action, order.orderType, order.totalQuantity, orderState.status)
                    ...
                    if self.flag_account_with_server and (
                            not flag_account_true):  # если номер аккаунта прочитан с сервера - сверяем
