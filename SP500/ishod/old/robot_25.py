...
class RobotApp(QThread, EWrapper, EClient):
    ...
    def openOrder(self, orderId, contract, order, orderState):
        ...
        if self.old_str_openOrder != self.str_openOrder:
            ...
            # orderState.equityWithLoanBefore, orderState.initMarginChange, orderState.maintMarginChange,
    ...
    def Trade(self, contract=Contract(), trd="", order_direction="", position=0, contract_price=0, orderType="LMT", real="real"):
        # order_direction = ""
        price_input = 0
    ...
    def run(self):
        ...
        dict_week_day = {0: "Monday ", 1: "Tuesday", 2: "Wednesday", 3: "Thursday", 4: "Friday", 5: "Saturday", 6: "Sunsday"}
        order_direction = "None"
        # self.day_candle_tmp = {a: {'dt': "", 'o': 0, 'h': 0, 'l': 0, 'c': 0} for a in range(1, self.NUM_DAY_CANDLE+1)}
        self.day_candle = {a: {'dt': "", 'o': 0, 'h': 0, 'l': 0, 'c': 0} for a in range(1, self.NUM_DAY_CANDLE + 1)}
        a = 0
        loss_1_lot = 0
        req = 1
        # try:
                ...
                while self.isConnected() or not self.msg_queue.empty():
                    ...
                    if flag_account_true:  # если аккаунт валидный
                            ...
                            if flag_time_paper:  # if time_paper:  #контракт читали и время бумаги уже не None
                                ...
                                if not self.flag_Position:
                                    if time_paper.hour == int(self.time_to_act['input_hour']) \
                                        ...
                                        number_of_lot = (
                                                1 - contract_subtractor)  # кол-во лотов, рассчитывается от риска - НАДО!!!
                                        if number_of_lot <= 0:
                                            number_of_lot = 0
                                ...
                                # мы в позе
                                    ...
                                    if time_paper.hour == int(self.time_to_act['output_hour']) \
                                        ...
                                        if flag_need_output:
                                            ...
                                            # number_of_lot = abs(self.position)
                                            # # price_input = 0
                                            # if self.position > 0:  # мы в лонгах
                                            #     order_direction = "SELL"  # продаем по цене ниже рынка
                                            #     price_input = ((self.contract_price * 9) // 10)
                                            # else:  # мы в шортах
                                            #     order_direction = "BUY"  # покупаем по цене выше рынка
                        ...
                        if self.flag_day_candle:
                            ...
                            if direction == "-L":   # если сегодня в шорт
                            ...
                            relative_stop = roundTick(cena=relative_stop, tick=self.minTick, param="+") # стоп округляем в большую сторону
                            ...
                            self.servicewindow.lbl_svv_Edit.setText(str(absolute_stop))
                            ...
                            if order_direction == "BUY":
                                ...
                                if absolute_stop >= (self.contract_price - (4*self.minTick)):
                                    ...
                                    print("Цена возле стопа или ниже: стоп =", absolute_stop, ", цена = ",
                                          (self.contract_price - (4*self.minTick)), ", сброс приказа в", order_direction)
                            ...
                            if order_direction != "None" and absolute_stop > 0:
                                ...
                                loss_1_lot = 0
                            print(f'Потеря на один лот = {loss_1_lot}')
                        ...
                        if self.flag_openOrder:
                            ...
                            self.servicewindow.lbl_go_Edit.setText(str(round(self.go, 2)))
