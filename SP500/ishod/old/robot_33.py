...
class RobotApp(QThread, EWrapper, EClient):
    ...
    def run(self):
        ...
        body_text_email = "None"
        # self.servicewindow.send_email(host_email, subject_email, self.to_addr_email,
        ...
        while True:
            ...
            if self.mainwindow.flag_connect:
                ...
                print(body_text_email)
                Interface.send_email(host_email, subject_email, self.to_addr_email,
                                     from_addr_email, password_email, body_text_email)
                time.sleep(0.1)
                ...
                while self.isConnected() or not self.msg_queue.empty():
                    ...
                    if flag_account_true:  # если аккаунт валидный
                            ...
                            if flag_time_paper:  # if time_paper:  #контракт читали и время бумаги уже не None
                                ...
                                if not self.flag_Position:
                                    if time_paper.hour == int(self.time_to_act['input_hour']) \
                                        ...
                                        if number_of_lot <= 0:
                                            number_of_lot = 0
                                        log_r.info(f'Направление {self.order_direction}, кол-во контрактов {number_of_lot}')
                                        self.MessageInfo(
                        ...
                        if self.flag_5_min_candle:
                            ...
                            time_tmp_min = datetime.fromtimestamp(int(time_tmp)).astimezone(pytz.timezone(self.contract_tz)).minute
