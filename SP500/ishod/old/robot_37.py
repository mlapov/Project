...
Пара =  файл robot_5 + Interface_5
 ...
 к выяснению и доработке
  ...
  - лицензия - файл mos.json - это по сути номер аккаунта - его надо будет читать наверное из файла какого-то - шифрованная запись
    ...
    - для телеграмма надо токен робота - узнаем при создании бота -  BotFather
    ...
    def run(self):
        ...
        while True:
            ...
            if self.mainwindow.flag_connect:
                ...
                # self.MessageEmail(subject="Тест2", to_addr=self.to_addr_email, text="Тест 2")
                ...
                while self.isConnected() or not self.msg_queue.empty():
                    ...
                    if self.mainwindow.close_app:
                        ...
                        print('stop1')
                        # self.telegram_interactive.bot.stop_polling()
                        print('stop2')
                        # self.telegram_interactive.bot.bot_stop()
                        print('stop3')
                        # self.telegram_interactive.bot.close()
                        print('stop31')
                        break
            ...
            # нажали кнопку закрытия робота - будем рассоединяться
                ...
                print('Закрыли Интерфейс6')
                log_r.info('Закрыли Интерфейс6')
                # self.cancelMktData(self.nextOrderId())
                # self.reqAccountUpdates(False, "")
                self.email.flag_run_email = False  # останавливаем поток
                self.telegram.flag_run_telegram = False
                # self.telegram_interactive.bot.close()  #.stop_bot()
                self.telegram_interactive.flag_run_telegram = False
                print('stop4')
                self.telegram_interactive.bot.stop_polling()
                print('stop5')
                # self.telegram_interactive.bot.bot_stop()
