...
class TelegraBot(Thread):
    def __init__(self):
        ...
        try:
            self.bot.send_message(987861324, "Привет2!!!!")
        except:
    ...
    def run(self):
        ...
        while self.flag_run_telegram:
            ...
            if self.flag_sent_telegram:
                self.flag_sent_telegram = False
                try:
                    self.bot.send_message(987861324, "Салют!!!!"+str(self.i))
                except:
                    print("ID не корректен. Введите корректный ID")
...
if __name__ == '__main__':
    ...
    sE = SendEmail()
    tB = TelegraBot()
    rb = robot.RobotApp(mainwindow=wU, servicewindow=wS, email=sE, telegram=tB)  # будем получать доступ к Окну
