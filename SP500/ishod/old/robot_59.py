...
Пара =  файл robot_5 + Interface_5
 ...
 к выяснению и доработке
  ...
  можно синхронизировать текуе часы робота и серверные - пробовал - но это гемор
    ...
    НАДО ПРОВЕРИТЬ В ВЫХОДНЫЕ - СДЕЛАНО !!!!! -если робот запущен в выходные то у нас НЕТ цены в робота - Price = self.contract_price -
    ...
    - IB Gatey попробывать, exe-шник сделать, другой комп, другую винду, Linux, сделать стандартные настроки
    - когда Ошибка соединения - связь с сервером ставить в NO
    - прогнать стратегию в ноябре
                        ...
                        if flag_account_true:  # если аккаунт валидный
                            ...
                            if self.flag_contract_price_change:
                                ...
                                if self.contract_price is not None:
                                    ...
                                    self.set_FromRobotAppToQT(param="lbl_paper_price_Edit",
                                                              text=f"{format(self.contract_price_close, '.2f')}")
