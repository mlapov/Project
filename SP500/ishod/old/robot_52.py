...
Пара =  файл robot_5 + Interface_5
 ...
 к выяснению и доработке
  ...
  можно синхронизировать текуе часы робота и серверные - пробовал - но это гемор
    ...
    - сделать прокрутку в Окне сообщение нормальное - чтобы само в низ моталось
...
class RobotApp(QThread, EWrapper, EClient):
    ...
    def error(self, reqId, errorCode, errorString, advancedOrderRejectJson):
        ...
        if advancedOrderRejectJson:
            ...
            logging.critical("error: Id:", reqId, "Code:", errorCode, "Msg:", errorString, "AdvancedOrderRejectJson:", advancedOrderRejectJson)
        else:
        ...
        while True:
            ...
            if self.mainwindow.flag_connect:
                ...
                try:
                    ...
                    print("v4")
                    tB2.send("Connect")  # .send_message(987861324, f"Коннект!!!! ") #self.telegram_id
                    ...
                    logging.warning('Ошибка соединения. Соединиться с сервером не удалось.')
                    ...
                    while self.isConnected() or not self.msg_queue.empty():
                        ...
                        if flag_account_true:  # если аккаунт валидный
                            ...
                            if self.flag_day_candle and self.position == 0:  # или self.flag_Position ?
                                ...
                                if self.risk > 0:
                                    min_depo = ((loss_1_lot * 100) / self.risk) * 1.02 if self.risk != 0 else 0  # немного увеличиваем минимальный депозит
                                else:
                                    min_depo = 0
            ...
            # мы разъединены c сервером
                ...
                # if self.servicewindow.flag_logging_level:
                #     self.servicewindow.flag_logging_level = False
                #
