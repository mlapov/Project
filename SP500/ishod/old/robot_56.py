...
Пара =  файл robot_5 + Interface_5
 ...
 к выяснению и доработке
  ...
  можно синхронизировать текуе часы робота и серверные - пробовал - но это гемор
    ...
    - иногда TWS - не может сам загрузиться - надо вводить пароль - надо либо оповещение слать
    либо как-то вводить пароль (хотя если в этом случае вводишь - TWS ругается и надо закрыть его и опять открыть)

    - торговля в Gate (не TWS)

    - выводить Ошибка соединения с сервером (когда пропадет коннект) - не в окно сообщений - засерается
    ...
    - !!!!! был какой-то бак-зависание: была ошибка соединения с сервером - TWS сам разорвал - а робот почему-то
    ...
    - проблема "9-06" - робот пытается соединиться - однако старое соединине еще действует
    ...
    - надо разобраться с потоками - сигналы и слоты в QT5
...
class RobotApp(QThread, EWrapper, EClient):
    ...
    ButtonConnectToQT = pyqtSignal(str)  # посылаем нажатие кнопки Коннект
    ...
    def ReadMOS(self):
        ...
        try:
            ...
            logging.critical('Файл mos.json отсутствует')  # log_r.critical('Файл mos.json отсутствует')
    ...
    def ReadCalendar(self):
        ...
        try:
            ...
            logging.critical('Файл calendar.json отсутствует')
    ...
    def ReadConfig(self):
        ...
        try:
            ...
            logging.critical('Файл config.json отсутствует')
        ...
        while True:
            ...
            if self.mainwindow.flag_connect:
                ...
                if (not flag_correct_config) or (not flag_correct_calendar) or \
                    ...
                    # self.signals_FromRobotAppToQT["btn_connect"] = "Connect"
                    # self.FromRobotAppToQT.emit(self.signals_FromRobotAppToQT["btn_connect"])
                    # self.set_FromRobotAppToQT(param="btn_connect", text="Connect")
                    continue
                ...
                try:
                    ...
                    print("v4")
                    tB2.send(
                        f"Connect - {datetime.now().strftime('%A, %d-%m-%Y, %H:%M:%S')}")  # .send_message(987861324, f"Коннект!!!! ") #self.telegram_id
                    ...
                    if not flag_error_connect:
                        tB2.send(
                            f"Ошибка соединения. Соединиться с сервером не удалось. - {datetime.now().strftime('%A, %d-%m-%Y, %H:%M:%S')}")
                            ...
                            if self.flag_5_min_candle:
                                ...
                                if time_tmp_min == int(self.time_to_act['input_min']):
                                    ...
                                    self.set_FromRobotAppToQT(param="lbl_price_input_trade_Edit",
                                                              text=f"{format(self.price_input_trade,'.2f')}")
                                    ...
                                    tmp_str = str(f"Trade entry (account {self.account}):\n"
                                                  ...
                                                  f"Estimated Entry Point: {self.price_input_trade}\n"
                                ...
                                if self.param_orderStatus["status"] == "Filled" and self.param_orderStatus[
                                    ...
                                    if flag_output:
                                        ...
                                        tmp_str = str(f"Trade exit (account {self.account}):\n"
                                                      f"Paper: {self.contract_1['localSymbol']}\n"
                                                      f"Estimated exit point: {self.contract_price}\n"
