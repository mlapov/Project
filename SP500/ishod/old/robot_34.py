...
Пара =  файл robot_5 + Interface_5
 ...
 к выяснению и доработке
  ...
  - лицензия - файл mos.json - это по сути номер аккаунта - его надо будет читать наверное из файла какого-то - шифрованная запись
    ...
    - если запустить робота + коннет без TWS - робот закрывается с ошибкой
"""
import json
import logging
import os
...
class RobotApp(QThread, EWrapper, EClient):
    ...
    #     print(f"4successfully sent email to {to_addr} from {from_addr}:\n{BODY}")
    #     # try:
    #     server.sendmail(from_addr, to_addr, BODY)
    #     # except:
    #     #     print("Сообщение на email послать не удалось!")
    #     print(f"5successfully sent email to {to_addr}: {body_text}")
    #     server.quit()
    #     print(f"6successfully sent email to {to_addr}: {body_text}")
    ...
    def run(self):
        ...
        subject_email = "Subscription"
        self.to_addr_email = "mikhaillapov@gmail.com"
        from_addr_email = "robot_sp@hotmail.com"
        password_email = "Abyrvalg44!"
        body_text_email = "begin"
        # BODY = "\r\n".join((
        #     "From: %s" % from_addr_email,
        #     "To: %s" % self.to_addr_email,
        #     "Subject: %s" % subject_email,
        #     "",
        #     body_text_email
        # ))
        # server = smtplib.SMTP(host_email)
        # server.starttls()
        # server.login(from_addr_email, password_email)
        # server.sendmail(from_addr_email, self.to_addr_email, BODY)
        # server.quit()
        print(self.email.em)
        print(self.email.i)
        self.email.i = 1000
        self.email.flag_sent_email = True
        # self.email.send_email(host_email, subject_email, self.to_addr_email,
        #                       from_addr_email, password_email, body_text_email)
        ...
        while True:
            ...
            if self.mainwindow.flag_connect:
                ...
                body_text_email = "Зашли во внешний цикл робота"
                # print(self.email.em)
                self.email.i = 1000
                self.email.flag_sent_email = True
                ...
                while self.isConnected() or not self.msg_queue.empty():
                            ...
                            if flag_time_paper:  # if time_paper:  #контракт читали и время бумаги уже не None
                                ...
                                if not self.flag_Position:
                                    if time_paper.hour == int(self.time_to_act['input_hour']) \
                                        ...
                                        self.Trade(contract=contract, trd="INPUT", order_direction=self.order_direction,
                    ...
                    if self.mainwindow.close_app:
                        ...
                        self.cancelMktData(self.nextOrderId())
                        self.reqAccountUpdates(False, "")
                        self.email.flag_run_email = False  # останавливаем поток
                        break
