...
class Interface(QMainWindow):
    ...
    def initWin(self):
        self.resize(460, 720)
        self.move(200, 200)  # координаты на Рабочем столе
...
class WinUser(Interface):
    ...
    def initWinUser(self):
        self.setWindowTitle("Robot S&P 500")

        self.centralWidget = QWidget()
        self.setCentralWidget(self.centralWidget)

        # картинка вращающаяся
        self.lbl_pic = QLabel('-', self)
        self.pixmap = QPixmap("config/matreshka.png")
        self.pixmap_45 = QPixmap("config/matreshka_45.png")

        # self.lbl_pic.resize(10, 10)
        ...
        field_connect.addWidget(self.lbl_pic)
        ...
        # точка входа
        self.lbl_price_input_trade = QLabel('Average Price input: ', self)
    ...
    def FromRobotApp(self, fromrobotapp):
        ...
        if self.deg_0:
            ...
            self.pixmap_45 = QPixmap(self.pixmap_45.transformed(t))
            self.lbl_pic.setPixmap(self.pixmap_45)
            self.deg_0 = True

        # if self.lbl_pic.text() == "-":
        #     self.lbl_pic.setText('\\')
        # elif self.lbl_pic.text() == "\\":
        #     self.lbl_pic.setText('|')
        # elif self.lbl_pic.text() == "|":
        #     self.lbl_pic.setText('/')
        # elif self.lbl_pic.text() == "/":
        #     self.lbl_pic.setText('-')
        # print(f"lbl_pic = {self.lbl_pic.text()}")
...
class WinService(Interface):
    ...
    def initWinService(self):
        ...
        self.move((200 + 460), 200)  # X, Y где 0 это сверху-слева, 460 - ширина Основного окна
        ...
        try:
            ...
            lbl_no_config = QLabel('Файл config/config.json отсутствует', self)
            ...
            try:
                if type(int(risk)) is int:
                    if int(risk) >= 0 and int(risk) <= 100:  #
                        ...
                        wU.MessageInfo(f"{datetime.now().strftime('%d.%m.%Y, %H:%M:%S - ')} Risk не в диапазоне (0-100): {risk}")
            except:
                ...
                wU.MessageInfo(datetime.now().strftime("%d.%m.%Y, %H:%M:%S - ") +
                               f"Недопустимое значение Risk`a: {risk}")
...
if __name__ == '__main__':
    ...
    app.setWindowIcon(QtGui.QIcon('config/matreshka.ico'))
    wU = WinUser()
    wS = WinService()
    sE = SendEmail()
